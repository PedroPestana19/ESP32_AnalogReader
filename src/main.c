...
#define SUPER_SAMPLING 15
#define SAMPLING_FREQ (FREQUENCY * SUPER_SAMPLING)
...
uint16_t samples[SUPER_SAMPLING];
bool pin = false;
bool led = false;
...
void app_main()
{
    ...
    {
        printf("I2S Event queue not initializes!!");
        while (pdTRUE)
            ;
    }
    ...
    if (Data_Queue == NULL)
    {
        printf("Data queue not initializes!!");
        while (pdTRUE)
            ;
    }
    setupI2S();
    ...
    while (pdTRUE)
    {
        if (xQueueReceive(I2S_Event_Queue, &evt, portMAX_DELAY) == pdPASS)
        {
            if (evt.type == I2S_EVENT_RX_DONE)
            {
                ...
                do
                {
                    i2s_read(I2S_PORT, samples + bytesRead, SUPER_SAMPLING * 2, &bytesRead, portMAX_DELAY);
                } while (bytesRead < SUPER_SAMPLING);
                if (!xQueueSend(Data_Queue, sampleFilter(samples, SUPER_SAMPLING), pdMS_TO_TICKS(2)))
                {
                    printf("Send error");
                    while (1)
                        ;
...
static void setupI2S(void)
{
    printf("Sample Rate = %d\nBits per Sample = %d\nNumber DMA Buffers = %d\nSize DMA per Buffer = %d\n",
           SAMPLING_FREQ, I2S_BITS_PER_SAMPLE_16BIT, DMA_BUF_COUNT, SUPER_SAMPLING);
    esp_err_t err;
